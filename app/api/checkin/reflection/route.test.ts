...
vi.mock('@/lib/db-helpers', () => ({
  ...
  createCheckin: vi.fn(),
  updateTodayStats: vi.fn(), // Reflection does not call this, but good to mock
...
describe('POST /api/checkin/reflection', () => {
  ...
  it('should create a reflection check-in successfully', async () => {
    const request = new NextRequest('http://localhost/api/checkin/reflection', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Test-User-Id': MOCK_USER.whopUserId,
      },
      body: JSON.stringify({ note: 'My reflection' }),
    });
    ...
    expect(dbHelpers.updateTodayStats).not.toHaveBeenCalled(); // Reflections do not update stats
  });
  ...
  it('should sanitize the note field', async () => {
    const request = new NextRequest('http://localhost/api/checkin/reflection', {
      ...
      body: JSON.stringify({ note: '<script>alert("xss")</script>My reflection' }),
    });

    await POST(request);

    expect(dbHelpers.createCheckin).toHaveBeenCalledWith(expect.objectContaining({
      note: '<script>alert("xss")</script>My reflection', // Sanitize is mocked to return input as-is
    }));
  });
});
